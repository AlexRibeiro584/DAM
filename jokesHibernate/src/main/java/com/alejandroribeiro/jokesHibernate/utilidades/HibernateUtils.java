package com.alejandroribeiro.jokesHibernate.utilidades;
...
import com.alejandroribeiro.jokesHibernate.entidades.Language;
	...
	public class HibernateUtils {
		 ...
		 * @return
		 */
		public static List<EntidadBD> devolverListaObjetos(Class<?> clase){
			if(clase.isInstance(Language.class)) {
				return (List<EntidadBD>)session.createNamedQuery("get_languages", clase).getResultList();
			}
			else if(clase.isInstance(Categories.class)) {
				return (List<EntidadBD>)session.createNativeQuery("SELECT * from categories").addEntity(clase).getResultList();
			}
			else {
				return (List<EntidadBD>)session.createSelectionQuery("from " + clase.getName(),clase).list();
			}
		}
		@SuppressWarnings("deprecation")
		public static List<?> devolverCategories(String parameter, String query){
			return session.createNativeQuery("SELECT * FROM categories WHERE :parameter like :query")
					.addEntity(Categories.class).setParameter("parameter", parameter)
		 ...
		 * @return
		 */
		public static List<Language> devolverListaLenguajes(String query){
			return session.createNamedQuery(query, Language.class).getResultList();
		}
		/**
		...
		public static boolean save(Object object) {
			...
			try {
				...
				System.out.println("Fila no insertada, ya existe una fila con esa clave primaria");
				trans.rollback();
				return false;
			} 
		}
		 ...
		 * @param claseItem Nombre del objeto de tipo clase.
		 * @param where Condición de búsqueda en la clase.
		 * @param item Objeto con los nuevos datos, actualiza todo lo que no sea null
		 ...
		 */
		public static <T> boolean update(Class<?> claseItem, String where, EntidadBD item) {
			Transaction trans = null;
			try {
				List<T> resultados = session.createQuery("FROM " + claseItem.getSimpleName() + " WHERE " + where).list();
				trans = session.beginTransaction();
				Arrays.asList(claseItem.getDeclaredFields()).forEach(f -> {
					resultados.forEach(r -> {
						f.setAccessible(true);
						try {
							if (f.get(item) != null && !Modifier.isStatic(f.getModifiers()))
								f.set(r, f.get(item));
